...
class CMD_NFT_Create {
    static parseTX(rtx) {
        ...
        const nft = AR_Parser_NFT.db.getNFT(output.symbol)
        if (nft && !testing) {
    ...
    static fillObj(nidObj, rtx) {
        ...
        const nft = AR_Parser_NFT.db.getNFT(symbol)
        if (nft && !testing){
        ...
        AR_Parser_NFT.db.nftCreate(rtx.output) //create or update the NFT
...
class CMD_NFT_Transfer {
    ...
    static _verifyTransfer(output,nidObj=null,objMap=null){
        ...
        if(nidObj==null) nidObj = AR_Parser_NFT.db.loadDomain(output.domain)
        ...
        for (const item of transInfo) {
            ...
            if(desObj==null)
                desObj = AR_Parser_NFT.db.loadDomain(item.to)
            if(!desObj){
...
module.exports = { Parser_NFT: AR_Parser_NFT }